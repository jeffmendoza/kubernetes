...
The `kubectl` line below first specifies the name of the [repilcation controller](/docs/replication-controller.md) to create `my-nginx`. Then, the [Docker image](https://registry.hub.docker.com/_/nginx/) to run `--image=nginx`. Next, the number of [pod](/docs/pods.md) replicas to create `--replicas=2`. Finally, the port to open for each pod `--port=80`.
